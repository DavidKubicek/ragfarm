...
def write_slot_env(slot: int, entry: dict) -> float:
    ...
    util = util_for(entry)
    # The reasoning parser is a property of the CHECKPOINT, not of the box, so it
    # belongs here and not in the global .env. A Thinking model needs it to split
    # <think> out of the stream; an Instruct model must NOT have it, because the
    # qwen3 parser keys on </think> and, finding none, files the entire answer as
    # reasoning and leaves content empty — a blank message with a four-minute
    # "thinking" trace holding the real output. The unit uses the word-splitting
    # $LLM_REASONING_ARGS form so an empty value here means no flag at all.
    thinking = entry.get("profile", "") != "vision-instruct"
    reasoning_args = "--reasoning-parser qwen3" if thinking else ""
...
# profile {entry.get('profile', '?')}: {'Thinking — split <think> into reasoning' if thinking else 'Instruct — NO reasoning parser, or the whole answer is filed as reasoning'}
LLM_REASONING_ARGS={reasoning_args}
