...
| mmproj     | none — text-only model. A vision-language swap (e.g. Qwen2.5-VL) needs a second GGUF here, path in `.env` `LLM_GGUF_MMPROJ`; see `scripts/fetch-llm.sh --list` and `docs/deployment.md` → "Model management". |
